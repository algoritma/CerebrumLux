...
UserIntent IntentRouter::detect(const std::string& text) {
    FastTextResult ftres = ft_->classify(text);

    // Basit bir eşleme, FastText etiketlerini UserIntent enum'una çevirir.
    // Etiketler FastText modeli eğitilirken belirlenmelidir.
    if (ftres.confidence < cfg_.fasttext_forward_threshold) {
        return UserIntent::Unknown; // Güven düşükse bilinmeyen olarak işaretle
    }

    if (ftres.label == "code") return UserIntent::Programming;
    if (ftres.label == "conversation") return UserIntent::Communication;
    if (ftres.label == "analysis") return UserIntent::Research; // Analysis -> Research
    if (ftres.label == "factual") return UserIntent::RequestInformation; // Factual -> RequestInformation
    if (ftres.label == "creative") return UserIntent::CreativeWork;
