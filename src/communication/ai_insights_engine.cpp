...
float AIInsightsEngine::calculate_autoencoder_reconstruction_error(const std::vector<float>& statistical_features) const {
    if (statistical_features.empty() || statistical_features.size() != CryptofigAutoencoder::INPUT_DIM) {
        LOG_MESSAGE(LogLevel::ERR_CRITICAL, std::wcerr, L"AIInsightsEngine::calculate_autoencoder_reconstruction_error: İstatistiksel özellik vektörü boş veya boyut uyuşmuyor. Yüksek hata döndürülüyor.\n");
...
std::vector<AIInsight> AIInsightsEngine::generate_insights(const DynamicSequence& current_sequence) {
    LOG_MESSAGE(LogLevel::DEBUG, std::wcerr, L"AIInsightsEngine::generate_insights: Icgoru uretimi basladi.\n");
    ...
    LOG_MESSAGE(LogLevel::DEBUG, std::wcerr, L"AIInsightsEngine::generate_insights: Icgoru uretimi bitti. Sayi: " << insights.size() << L"\n");
    return insights;
}
