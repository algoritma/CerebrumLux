...
void CryptofigAutoencoder::initialize_random_weights() {
    ...
    LOG_MESSAGE(LogLevel::INFO, std::wcout, L"CryptofigAutoencoder: Agirliklar rastgele baslatildi.\n");
}
...
std::vector<float> CryptofigAutoencoder::encode(const std::vector<float>& input_features) const {
    if (input_features.size() != INPUT_DIM) {
        LOG_MESSAGE(LogLevel::ERR_CRITICAL, std::wcerr, L"CryptofigAutoencoder::encode: Girdi boyutu uyuşmuyor! Beklenen: " << INPUT_DIM << L", Gelen: " << input_features.size() << L"\n");
...
std::vector<float> CryptofigAutoencoder::decode(const std::vector<float>& latent_features) const {
    if (latent_features.size() != LATENT_DIM) {
        LOG_MESSAGE(LogLevel::ERR_CRITICAL, std::wcerr, L"CryptofigAutoencoder::decode: Gizil boyut uyuşmuyor! Beklenen: " << LATENT_DIM << L", Gelen: " << latent_features.size() << L"\n");
...
void CryptofigAutoencoder::adjust_weights_on_error(const std::vector<float>& input_features, float learning_rate_ae) {
    if (input_features.size() != INPUT_DIM) {
        LOG_MESSAGE(LogLevel::ERR_CRITICAL, std::wcerr, L"CryptofigAutoencoder::adjust_weights_on_error: Girdi boyutu uyuşmuyor! Ayarlama yapılamıyor.\n");
        return;
    }
    ...
    if (total_error > 0.1f) { // Örnek eşik değeri
        ...
        LOG_MESSAGE(LogLevel::DEBUG, std::wcerr, L"CryptofigAutoencoder: Agirliklar hataya gore ayarlandi. Hata: " << std::fixed << std::setprecision(4) << total_error << L"\n");
    }
}
...
void CryptofigAutoencoder::save_weights(const std::wstring& filename) const {
    ...
    if (!fp) {
        LOG_MESSAGE(LogLevel::ERR_CRITICAL, std::wcerr, L"Hata: Autoencoder agirlik dosyasi yazilamadi: " << filename << L" (errno: " << errno << L")\n");
        return;
    }
    ...
    fclose(fp);
    LOG_MESSAGE(LogLevel::INFO, std::wcout, L"CryptofigAutoencoder agirliklari kaydedildi: " << filename << L"\n");
}

void CryptofigAutoencoder::load_weights(const std::wstring& filename) {
    FILE* fp = _wfopen(filename.c_str(), L"rb"); // Binary modda oku
    if (!fp) {
        LOG_MESSAGE(LogLevel::WARNING, std::wcerr, L"Uyari: Autoencoder agirlik dosyasi bulunamadi, rastgele agirliklar kullaniliyor: " << filename << L" (errno: " << errno << L")\n");
    ...
    if (loaded_input_dim != INPUT_DIM || loaded_latent_dim != LATENT_DIM) {
        LOG_MESSAGE(LogLevel::ERR_CRITICAL, std::wcerr, L"Hata: Autoencoder agirlik dosyasi boyutlari uyuşmuyor! Dosya: " << loaded_input_dim << L"x" << loaded_latent_dim << L", Beklenen: " << INPUT_DIM << L"x" << LATENT_DIM << L". Rastgele agirliklar kullaniliyor.\n");
        fclose(fp);
        initialize_random_weights();
        return;
    }
    ...
    fclose(fp);
    LOG_MESSAGE(LogLevel::INFO, std::wcout, L"CryptofigAutoencoder agirliklari yuklendi: " << filename << L"\n");
}